/*  Author(s): Shreyansh Gandhi (ID: 2012A7PS070P), Himanshu Sharma(ID: 2012C6PS545P). Team Number = 36, Group No = 2.
  ...
  3. Abstract Syntax Tree (Incomplete)*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
struct parseNode{//node of a parse tree: n-ary tree
  char *s;      //points to a string like NTPROG, or TMAIN, TWHILE.
...
char *type,*structs,*args,*decs,*funcs,*lBrace,*rBrace,*lPar,*rPar,*semi;
...
void displayTree(struct parseNode *root,int spaces,struct parseNode *p)
{
    //preorder traversal.
    if(!root)
        return;
    ...
    if((root->s)[0]=='N'){
        ...
        if(root->childCount == 0)
            ...
            p = root;
            for(i=(10-root->childCount);i<=9;i++)
              displayTree(root->children[i],spaces+1,p);
        }
    }
    else{
      printf("No Children \n");
...
void correctArray(struct parseNode *root){
  ...
  root->childCount = (short int)count;
  i = (10-count);
...
void pruneExtraNT(struct parseNode *root)   //working.
{
  if(!root)
    return;
  int i;
  ...
  if(!parent) //the root node.
    return;
  if(parent->childCount==1){
    printf("Extra: Removing %s\n",parent->s);
    parent->s = root->s;
    parent->childCount = root->childCount;
    for(i=0;i<10;i++)
      parent->children[i] = root->children[i];  //now current node's children become parent's children.
    free(root);
  }
}

void removeEpsilons(struct parseNode *root) //going wrong somewhere...
{
  if(!root)
    return;
  if(root->s[0]=='T')   //the node is a token.
    return;
  int i,j;
  short int count = root->childCount;
  for(i=(10-count);i<=9;i++)
